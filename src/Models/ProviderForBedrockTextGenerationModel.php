...
namespace AiProviderForBedrock\Models;
...
/**
 * Class for a Bedrock Claude text generation model.
 *
 * @since 1.0.0
 *
 ...
 * }
 */
class ProviderForBedrockTextGenerationModel extends AbstractApiBasedModel implements TextGenerationModelInterface
{
    /**
    ...
    final public function generateTextResult(array $prompt): GenerativeAiResult
    {
        ...
        $request = new Request(
            HttpMethodEnum::POST(),
            ProviderForBedrock::url('model/' . $this->metadata()->getId() . '/invoke'),
            $headers,
            $params,
    ...
    /**
     * Extracts a human-readable error message from an AWS error response body.
     *
     * @since 1.0.0
     *
     * @param string $body The raw response body.
     * @return string|null The error message, or null if not an AWS error format.
     */
    private static function getAwsErrorMessage(string $body): ?string
    {
        $data = json_decode($body, true);
        if (!is_array($data)) {
            return null;
        }
        $type = $data['__type'] ?? $data['type'] ?? '';
        $message = $data['message'] ?? $data['Message'] ?? '';
        if ($type === '' && $message === '') {
            return null;
        }
        return trim($type . ': ' . $message, ': ');
    }
